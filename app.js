...
function dashboardOperador(p) {
  ...
  const enriquecidas = pendentes.map(op => {
    ...
    return { ...op, produto_descricao: prod ? prod.descricao : op.produto_master_id };
  });
  const hoje = new Date();
...
function dashboardAdmin(p, sessao) {
  ...
  apont.forEach(ap => {
    const prod = produtos.find(pr => pr.id === ap.produto_final_id);
    const key = ap.produto_final_id;
    if (!resumoProdutos[key]) resumoProdutos[key] = { produto: prod ? prod.descricao : key, total_unidades: 0, opsMap: {} };
    const qtde = parseFloat(ap.quantidade_produzida) || 0;
    resumoProdutos[key].total_unidades += qtde;

    const opInfo = ops.find(o => o.id === ap.op_id);
    const numeroOp = opInfo ? opInfo.numero_op : (ap.op_id || '-');
    if (!resumoProdutos[key].opsMap[numeroOp]) resumoProdutos[key].opsMap[numeroOp] = { numero_op: numeroOp, quantidade: 0 };
    resumoProdutos[key].opsMap[numeroOp].quantidade += qtde;
  });

  const producaoDetalhada = Object.values(resumoProdutos).map(p => ({
    produto: p.produto,
    total_unidades: p.total_unidades,
    detalhe: Object.values(p.opsMap).sort((x, y) => String(x.numero_op).localeCompare(String(y.numero_op)))
  }));
...
function inicializarPlanilha() {
  const abas = {
    ...
    produtos: ['id','codigo','descricao','tipo','peso_master_kg','produto_master_id','peso_final','embalagem','custo_embalagem','custo_rotulo','tipo_embalagem','valor_cobrado_unidade','ativo'],
